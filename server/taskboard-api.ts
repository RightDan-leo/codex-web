  ...
  api.post("/taskboard/projects", (req, res) => {
    ...
    try {
      ...
      if (executor.kind === "remote") {
        const project = store.executionProject(executor.projectId, session.user_id);
        const remoteExecutor = project?.executor_id.startsWith("remote:") ? gateway.executor(project.executor_id) : undefined;
        if (!project || !remoteExecutor || remoteExecutor.status !== "online") {
  ...
  api.post("/taskboard/tasks/:id/start", (req, res) => {
    ...
    try {
      ...
      if (task.executor_kind === "remote") {
        const project = task.remote_project_id ? store.executionProject(task.remote_project_id, session.user_id) : undefined;
        const executor = project?.executor_id.startsWith("remote:") ? gateway.executor(project.executor_id) : undefined;
        if (!project || !executor || executor.status !== "online") {
