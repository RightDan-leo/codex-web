...
import type { AppDatabase, JobRow, ProjectRow, StoredAgentSelection } from "./db.js";
import {
  canTransitionTask,
...
export class TaskboardStore {
  ...
  executionProject(projectId: string, userId: string): ProjectRow | undefined {
    const project = this.db.getProject(projectId);
    return project?.user_id === userId && !project.archived_at ? project : undefined;
  }
  ...
  createTasks(projectId: string, userId: string, inputs: CreateTaskInput[]): TaskboardTaskRow[] {
    const project = this.requireProject(projectId, userId);
    ...
    const now = new Date().toISOString();
    const firstPosition = this.nextPosition(projectId, "backlog");
    ...
    try {
      ...
      inputs.forEach((input, index) => {
        ...
        insert.run(
          id, projectId, input.parentTaskId ?? null, input.title, input.description, input.priority, input.risk,
          input.estimatePoints ?? null, input.acceptanceCriteria, firstPosition + index, input.conversationId ?? null,
          project.executor_kind, project.remote_project_id, now, now,
        );
        this.appendEvent(projectId, id, userId, "task.created", {
          title: input.title, priority: input.priority, risk: input.risk,
    ...
    return ids.map((id) => this.getTask(id, userId)!);
  }
  ...
  startTask(id: string, userId: string, version: number, selection: StoredAgentSelection): StartedTaskboardTask {
    ...
    const executionProject = task.executor_kind === "remote"
      ? (task.remote_project_id ? this.executionProject(task.remote_project_id, userId) : undefined)
      : this.db.getDefaultProject(userId);
    if (!executionProject) throw new TaskboardConflictError("任务对应的执行项目不存在或已归档。");
    if (task.executor_kind === "remote" && !executionProject.executor_id.startsWith("remote:")) {
      throw new TaskboardConflictError("任务对应的远端执行项目配置无效。");
    }
    this.db.sqlite.exec("BEGIN IMMEDIATE");
    try {
      if (!task.conversation_id) {
        const sidebar = this.db.sqlite.prepare(`
          SELECT COALESCE(MAX(sidebar_order),0)+1 AS value FROM conversations
          WHERE user_id=? AND project_id IS ? AND deleted_at IS NULL AND archived_at IS NULL
        `).get(userId, executionProject.id) as { value: number };
        this.db.sqlite.prepare(`
          INSERT INTO conversations(
            id,user_id,project_id,title,title_source,agent_model,reasoning_effort,status,sidebar_order,
            has_unread_result,last_active_at,archived_at,deleted_at,created_at,updated_at
          ) VALUES(?,?,?,?,'manual',?,?,'idle',?,0,?,NULL,NULL,?,?)
        `).run(
          conversationId, userId, executionProject.id, task.title, selection.model, selection.reasoningEffort,
          sidebar.value, now, now, now,
        );
      }
      this.db.sqlite.prepare(`
  ...
  private validateConversation(project: TaskboardProjectRow, conversationId: string, userId: string): void {
    ...
    const conversationProject = conversation.project_id ? this.db.getProject(conversation.project_id) : undefined;
    const matches = project.executor_kind === "remote"
      ? conversation.project_id === project.remote_project_id && Boolean(conversationProject?.executor_id.startsWith("remote:"))
      : Boolean(conversationProject && !conversationProject.executor_id.startsWith("remote:"));
    if (!matches) {
