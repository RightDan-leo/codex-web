...
export function TaskboardPage({
  ...
  const [boardProjects, setBoardProjects] = useState<TaskboardProject[]>([]);
  const [selectedProjectId, setSelectedProjectId] = useState<string | null>(null);
  const [detail, setDetail] = useState<TaskboardProjectDetail | null>(null);
  const [executionProjects, setExecutionProjects] = useState<Project[]>([]);
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api.taskboardProjects(), api.projects()])
      .then(([projectResult, projectCatalog]) => {
        if (!active) return;
        setBoardProjects(projectResult.projects);
        setExecutionProjects(projectCatalog.projects);
  ...
  useEffect(() => {
    ...
    api.taskboardProject(selectedProjectId)
      .then((value) => {
        if (!active) return;
        setDetail(value);
        setBoardProjects((current) => current.map((project) => project.id === value.project.id ? value.project : project));
      })
  ...
  const online = detail ? executorOnline(detail.project.executor, executionProjects) : true;
  ...
  async function createProject(input: { name: string; description: string; executor: ExecutorTarget }) {
    ...
    try {
      ...
      setBoardProjects((current) => [result.project, ...current]);
  ...
  async function startTask(task: TaskboardTask) {
    if (!executorOnline(task.executor, executionProjects)) {
  ...
  async function archiveProject() {
    ...
    try {
      ...
      const remaining = boardProjects.filter((project) => project.id !== detail.project.id);
      setBoardProjects(remaining);
      setDetail(null);
  ...
  async function openWorkConversation(task: TaskboardTask) {
    if (!detail || !executorOnline(task.executor, executionProjects)) {
    ...
    try {
      if (!conversationId) {
        const created = await api.createConversation(task.executor.kind === "remote" ? task.executor.projectId : undefined, false);
  ...
  if (loading && boardProjects.length === 0) return <section className="taskboard-loading"><LoaderCircle className="spin" size={24} /><span>正在加载项目看板…</span></section>;
  ...
  return <section className="taskboard-page">
    <header className="taskboard-header">
      ...
      <div className="taskboard-header-actions">
        {boardProjects.length > 0 && <label className="taskboard-project-select"><span>当前项目</span><select value={selectedProjectId ?? ""} onChange={(event) => setSelectedProjectId(event.target.value)}>
          {boardProjects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}
    ...
    {!detail ? <div className="taskboard-empty">
      ...
      <div className="taskboard-project-summary">
        ...
        <div className="taskboard-project-chips">
          <span className={online ? "online" : "offline"}>{detail.project.executor.kind === "tenant" ? <HardDrive size={14} /> : <Monitor size={14} />}{executorLabel(detail.project.executor, executionProjects)}</span>
    ...
    {projectFormOpen && <ProjectForm projects={executionProjects} busy={busy} onCancel={() => setProjectFormOpen(false)} onSubmit={createProject} />}
    ...
    {selectedTask && detail && <TaskDetailDialog key={`${selectedTask.id}:${selectedTask.version}`} task={selectedTask} detail={detail} busy={busy}
      online={executorOnline(selectedTask.executor, executionProjects)} onClose={() => setSelectedTaskId(null)} onSave={saveTask}
...
function ProjectForm({ projects, busy, onCancel, onSubmit }: {
  projects: Project[];
  busy: boolean;
  onCancel: () => void;
  ...
  const remoteProjects = useMemo(() => {
    ...
    return projects.filter((project) => project.executor_id.startsWith("remote:"))
      .map((project) => ({ id: project.id, name: project.display_name || project.name, worker: project.machine_name, online: project.executor_status === "online" }))
  ...
  return <div className="taskboard-dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}>
    <form className="taskboard-dialog compact" onSubmit={submit}>
      ...
      <label>执行位置<select value={executorValue} onChange={(event) => setExecutorValue(event.target.value)}>
        ...
        {remoteProjects.map((project) => <option key={project.id} value={`remote:${project.id}`} disabled={!project.online}>{project.name} · {project.worker}{project.online ? "" : "（离线）"}</option>)}
      </select></label>
...
function executorLabel(target: ExecutorTarget, projects: Project[]): string {
  if (target.kind === "tenant") return "隔离 Tenant";
  const project = projects.find((candidate) => candidate.id === target.projectId);
  if (project) return `${project.display_name || project.name} · ${project.machine_name}`;
